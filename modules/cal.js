...
function Calendar(stringFullDay) {
	...
	var drawCal = function(nYear, nMonth, nDay) {
		...
		cal.onmouseout = function(event) {
			if (timeOut == null)
				timeOut = setTimeout(self.close, 500);
		};
		...
		for ( var i = 0; i < 6; i++) {
			...
			for ( var j = 0; j < 7; j++) {
				...
				if (display) {
					...
					if (date.getFullYear() == nYear && (date.getMonth() + 1) == nMonth && date.getDate() == firstDate) {
						td.style.border = "1px solid #ff0000";
					}

					td.style.cursor = "pointer";
					td.title = nYear + separator + dateFormatConversion(nMonth) + separator + dateFormatConversion(firstDate);
	...
	this.toDate = function() {
		var time = date.getFullYear() + "-" + numberToString(date.getMonth() + 1) + "-" + numberToString(date.getDate());
		return time;
	};
...
function SelectBox(nWidth) {
	...
	var inputBox = null;
	{
		...
		selectBoxObj.onmouseout = function(event) {
			if (timeOut == null)
				timeOut = setTimeout(hideOptions, 10);
		};
	...
	var rollbackOptions = function() {
		for ( var i = Number(inputBox.innerHTML) - 3, j = 1; i <= Number(inputBox.innerHTML) + 3; i++, j++) {
